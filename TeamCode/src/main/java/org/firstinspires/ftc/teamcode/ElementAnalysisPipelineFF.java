...
public class ElementAnalysisPipelineFF extends OpenCvPipeline {
    /*
     ...
     * Colors
     */
    static final Scalar BBLUE = new Scalar(3, 148, 252);
    static final Scalar PINK = new Scalar(158, 52, 235);
    ...
    ArrayList<ElementAnalysisPipelineFF.AnalyzedElement> internalElementList = new ArrayList<>();
    volatile ArrayList<ElementAnalysisPipelineFF.AnalyzedElement> clientElementList = new ArrayList<>();
    ...
    ElementAnalysisPipelineFF.Stage[] stages = ElementAnalysisPipelineFF.Stage.values();
    ...
    public ArrayList<ElementAnalysisPipelineFF.AnalyzedElement> getDetectedElements()
    {
    ...
    void analyzeContour(MatOfPoint contour, Mat input)
    {
        ...
        if (rotatedRectFitToContour.size.width > 60 && rotatedRectFitToContour.size.height > 30) {
            ...
            ElementAnalysisPipelineFF.ContourRegionAnalysis aboveMidlineMetrics = analyzeContourRegion(aboveMidline);
            ElementAnalysisPipelineFF.ContourRegionAnalysis belowMidlineMetrics = analyzeContourRegion(belowMidline);
            ...
            ElementAnalysisPipelineFF.AnalyzedElement analyzedElement = new ElementAnalysisPipelineFF.AnalyzedElement();
            ...
            analyzedElement.object = ElementAnalysisPipelineFF.ObjectType.ALLIANCE_SHIPPING_ELEMENT;
            ...
            if (rotatedRectFitToContour.center.x <= DIVISION_ONE) {
                analyzedElement.section = ElementAnalysisPipelineFF.Section.LEFT;
            } else if (rotatedRectFitToContour.center.x > DIVISION_ONE && rotatedRectFitToContour.center.x < DIVISION_TWO) {
                analyzedElement.section = ElementAnalysisPipelineFF.Section.MID;
            } else if (rotatedRectFitToContour.center.x >= DIVISION_TWO) {
                analyzedElement.section = ElementAnalysisPipelineFF.Section.RIGHT;
            }
    ...
    static ElementAnalysisPipelineFF.ContourRegionAnalysis analyzeContourRegion(ArrayList<Point> contourPoints)
    {
        ...
        if(hullMatOfInt.toArray().length > 0)
        {
            ...
            ElementAnalysisPipelineFF.ContourRegionAnalysis analysis = new ElementAnalysisPipelineFF.ContourRegionAnalysis();
